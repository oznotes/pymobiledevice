...
from pymobiledevice.util import readHomeFile, write_file, writeHomeFile
...
class LockdownClient(object):
    ...
    def validate_pairing(self):
        ...
        if pair_record:
            self.logger.info(
                "Using iTunes pair record: %s.plist", self.identifier)
            certPem = pair_record["HostCertificate"].data
            privateKeyPem = pair_record["HostPrivateKey"].data

        else:
            self.logger.warn(
                "No iTunes pairing record found for device %s", self.identifier)
            ...
            if record:
                ...
                self.logger.info(
                    "Found pymobiledevice pairing record for device %s", self.udid)
                certPem = pair_record["HostCertificate"].data
                privateKeyPem = pair_record["HostPrivateKey"].data
            else:
                self.logger.warn(
                    "No  pymobiledevice pairing record found for device %s", self.identifier)
                return False
    ...
    def startServiceWithEscrowBag(self, name, escrowBag=None):
        ...
        self.c.sendPlist({"Label": self.label, "Request": "StartService",
                          "Service": name, 'EscrowBag': escrowBag})
        StartService = self.c.recvPlist()
        if not StartService or StartService.get("Error"):
...
if __name__ == "__main__":
    ...
    l = LockdownClient()
    if l:
        n = writeHomeFile(HOMEFOLDER, "%s_infos.plist" %
                          l.udid, plistlib.writePlistToString(l.allValues))
